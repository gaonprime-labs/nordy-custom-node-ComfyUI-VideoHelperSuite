...
function initHelpDOM() {
  ...
  helpDOM.addHelp = function (node, nodeType, description) {
    ...
    node.computeSize = function () {
      ...
      size[0] = Math.max(
        size[0],
        title_width + LiteGraph.NODE_TITLE_HEIGHT * 2
      );
      return size;
    };
...
function initializeLoadFormat(nodeType, nodeData) {
  ...
  chainCallback(nodeType.prototype, "onNodeCreated", function () {
    ...
    chainCallback(formatWidget, "callback", function (value) {
      ...
      if ("frames" in format) {
        format.frame_load_cap = {
          step: format.frames[0] * 10,
          mod: format.frames[1],
        };
      }
...
function addVideoPreview(nodeType, isInput = true) {
  chainCallback(nodeType.prototype, "onNodeCreated", function () {
    ...
    previewWidget.updateSource = function () {
      ...
      //   }
      //   let qurl = api.apiURL(
      //     "/vhs/queryvideo?" + new URLSearchParams(previewWidget.value.params)
      //   );
      //   let query = undefined;
      //   try {
      ...
      // };
      // doQuery();
      // this.videoEl.src = previewWidget.value.params.fullpath;
      // this.imgEl.src = previewWidget.value.params.fullpath;
    };
  ...
  chainCallback(
    ...
    function (_, options) {
      ...
      if (previewWidget.videoEl?.hidden == false && previewWidget.videoEl.src) {
        if (
          ["input", "output", "temp"].includes(previewWidget.value.params.type)
        ) {
      ...
      if (this.video_query?.source) {
        let info_string =
          this.video_query.source.size.join("x") +
          "@" +
          this.video_query.source.fps +
          "fps " +
          this.video_query.source.frames +
          "frames";
...
function addFormatWidgets(nodeType, nodeData) {
  chainCallback(nodeType.prototype, "onNodeCreated", function () {
    ...
    chainCallback(formatWidget, "callback", (value) => {
      const formats =
        LiteGraph.registered_node_types[this.type]?.nodeData?.input?.required
          ?.format?.[1]?.formats;
      let newWidgets = [];
      if (formats?.[value]) {
...
function searchBox(event, [x, y], node) {
  ...
  async function updateOptions() {
    ...
    if (last_path != path) {
      ...
      let optionsURL = api.apiURL(
        "/vhs/getpath?" + new URLSearchParams(params)
      );
      try {
    ...
    if (event.click_time < 200 && delta == 0) {
      ...
      input.addEventListener("keydown", (e) => {
        if (e.keyCode == 9) {
          ...
          if (node.widgets[++i]?.type == "BOOLEAN") {
            //restrict to annotatedNUmbers
...
app.registerExtension({
  ...
  settings: [
    {
      ...
      tooltip:
        "Automatically transcode previews on request. Required for advanced functionality",
      type: "combo",
    ...
    },
    {
      ...
      tooltip:
        "Advanced previews have their resolution downscaled to the node size for performance. While a node can be resized to increase preview quality, a minimum width can be set that previews won't be downscaled beneath. Preveiws will never be upscaled, so this can safely be set large.",
      type: "number",
      attrs: {
        min: 0,
        step: 1,
    ...
    },
    {
      ...
      name: "Deadline",
      tooltip:
        "Determines how much time can be spent when encoding advanced previews. Realtime results in reduced quality, but good will likely cause the preview to stutter as initial generation occurs",
      type: "combo",
  ...
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    ...
    if (nodeData?.name == "VHS_LoadImages") {
      ...
      nodeData?.name == "VHS_LoadVideoPath" ||
      nodeData?.name == "VHS_LoadVideoFFmpegPath"
    ) {
      chainCallback(nodeType.prototype, "onNodeCreated", function () {
        const pathWidget = this.widgets.find((w) => w.name === "video");
        chainCallback(pathWidget, "callback", (value) => {
      ...
      chainCallback(nodeType.prototype, "onNodeCreated", function () {
        ...
        let fetch_files = async () => {
          ...
          options = options.filter(
            (file) =>
              file.startsWith(remainder) && file.endsWith(this.widgets[1].value)
          );
          if (
            options.length &&
            this.latest_file != options[options.length - 1]
          ) {
      ...
      VHSPATH(node, inputName, inputData) {
        let w = {
          ...
          draw: function (ctx, node, widget_width, y, H) {
            ...
            var show_text =
              app.canvas.ds.scale >=
              (app.canvas.low_quality_zoom_threshold ?? 0.5);
            var margin = 15;
          ...
          format_path: function (path, len = 30) {
            ...
            let isAbs = path[0] == "/";
            let partial = path.substr(
              path.length - (isAbs ? len - 2 : len - 1)
            );
  ...
  async setup() {
    ...
    let graphToPrompt = async function () {
      ...
      res.workflow.extra["VHS_latentpreview"] =
        app.ui.settings.getSettingValue("VHS.LatentPreview");
      res.workflow.extra["VHS_latentpreviewrate"] =
        app.ui.settings.getSettingValue("VHS.LatentPreviewRate");
      res.workflow.extra["VHS_MetadataImage"] =
        app.ui.settings.getSettingValue("VHS.MetadataImage");
      res.workflow.extra["VHS_KeepIntermediate"] =
        app.ui.settings.getSettingValue("VHS.KeepIntermediate");
      return res;
    };
...
api.addEventListener("VHS_latentpreview", ({ detail }) => {
  ...
  let previewWidget = previewNode.widgets.find(
    (w) => w.name == "vhslatentpreview"
  );
  if (!previewWidget) {
    let canvasEl = document.createElement("canvas");
    previewWidget = previewNode.addDOMWidget(
      "vhslatentpreview",
      "vhscanvas",
      canvasEl,
      {
  ...
  animateInterval = setInterval(() => {
    ...
    if (!ctx) {
      previewWidget.aspectRatio =
        previewImages[displayIndex].width / previewImages[displayIndex].height;
